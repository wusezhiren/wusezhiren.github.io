...
SKILLS = {
    "blade": [
        ...
        dict(key="s", name="破军升龙击", skl="chargecrash", hits=2,
             units=lambda r: r[0] + r[1], est="冲撞+上斩两段(原版突进后上挑)"),
        dict(key="d", name="猛龙断空斩", skl="rapidmoveslash", hits=5,
             units=lambda r: r[0] * 2 + r[1], est="后续%×2段+终结%(段数上限列缺失)"),
        dict(key="f", name="幻影剑舞", skl="illusionslash", hits=10,
             units=lambda r: r[0] * 10 + r[1] + r[2],
             est="斩击%×10(原版lv1最小斩击次数10)+剑气两段"),
    ...
    "berserk": [
        dict(key="z", name="十字斩", skl="gorecross", hits=2, units=lambda r: r[0] + r[1],
             est="斩击+血十字两段(原版血十字为独立判定)"),
        dict(key="a", name="崩山击", skl="hopsmash", hits=2, units=lambda r: r[0] + r[1],
             est="跃劈+冲击波(原版多段攻击2~3次取最小值)"),
        dict(key="s", name="嗜魂之手", skl="grabblastblood", hits=1,
             units=lambda r: r[0] + r[1]),
        dict(key="d", name="怒气爆发", skl="bloodblast", hits=4,
             units=lambda r: r[0] + r[2], est="本体+冲击波; 原版对周围敌人攻击4次"),
        dict(key="f", name="嗜魂封魔斩", skl="bloodyrave", hits=1, units=lambda r: r[0]),
        dict(key="g", name="崩山裂地斩", skl="outragebreak", hits=7,
             units=lambda r: r[0] + r[1] + r[2], hp_cost=lambda r: r[3],
             est="挥斩+冲击波+龟裂; 原版龟裂多段6次→挥斩1+龟裂6共7段; "
                 "HP消耗为 level info 第4列(原400)"),
    ],
    "spectre": [
        ...
        dict(key="a", name="满月斩", skl="moonlightslashfull", stat_skl="moonlightslash",
             hits=2, units=lambda r: r[0] + r[1], units_skl="moonlightslash",
             est="月形斩击+单手上斩两段(原版为近身弧月挑空斩, 非投射)"),
        dict(key="s", name="冰霜之萨亚", skl="saya", hits=6,
             units=lambda r: r[2] * 3, est="90%/秒×约3秒接触; 原版4.5秒持续冰冻场→6跳"),
        dict(key="d", name="瘟疫之罗刹", skl="epidemicrasa", hits=6,
             units=lambda r: r[3] * 0.5, est="中毒总伤×50%; 原版持续毒场→6跳"),
        dict(key="f", name="鬼影闪", skl="ghoststepslash", hits=5, units=lambda r: r[0]),
        dict(key="g", name="死亡墓碑", skl="tombstonerain", hits=6,
             units=lambda r: r[3] * 6, est="单墓碑魔攻×约6次命中; 原版3秒墓碑雨→6跳"),
    ],
    "asura": [
        ...
        dict(key="s", name="爆炎·波动剑", skl="firewave", hits=2,
             units=lambda r: r[0] + r[1], est="火波+爆炸两段"),
        ...
        dict(key="g", name="不动明王阵", skl="wavespinarea", hits=9,
             units=lambda r: r[0] * 8 + r[1], est="焰珠×8跳+爆炸; 原版4秒旋转阵→9跳均分"),
    ],
}
